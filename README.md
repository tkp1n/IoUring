...
2. Check the client-socket-queue. This `ConcurrentQueue` contains sockets to client endpoints for which a connect is in progress. For each connection in this queue, a `poll` (`IORING_OP_POLL_ADD`) for "writability" (`POLLOUT`) is added to the `io_uring`. "Writabilit" will indicate the completion of a `connect`.
